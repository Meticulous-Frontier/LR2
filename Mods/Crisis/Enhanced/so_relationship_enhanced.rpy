init 2 python:
    ...
    def so_relationship_quarrel_requirement(person):
        if side_character_is_unavail(person):
            return False
    ...
    def get_so_relationship_improve_person():
        potential_people = []
        for person in [x for x in known_people_in_the_game(excluded_people = unique_character_list + side_character_unavail_list()) if not x.relationship == "Married" and not x.has_role([girlfriend_role, affair_role])]:
    ...
    def get_so_relationship_worsen_person():
        potential_people = []
        for person in [x for x in known_people_in_the_game(excluded_people = unique_character_list + side_character_unavail_list()) if not x.relationship == "Single" and not x.has_role([affair_role])]:
            if person.relationship in relationship_stats and person.love > relationship_stats[person.relationship] - (person.get_opinion_score("cheating on men") * 5):
                potential_people.append(person)
        return get_random_from_list(potential_people)
