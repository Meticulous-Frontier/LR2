# This changes the implementation for the relationship array to use a key mapping table instead of actual person objects
...
init -1 python:
    ...
    class Relationship(renpy.store.object): #A class used to store information about the relationship between two people. Do not manipulate directly, use RelationshipArray to change things.
        def __init__(self, person_a_identifier, person_b_identifier, type_a, type_b = None, visible = None):
            ...
            self.type_a = type_a #person_a_identifier TO person_b_identifier, written so you could tell what person_b_identifier is if you listed them. Ie. "Lily - Daughter".
        ...
        def get_other_person(self, the_person): #Used to make it simpler to get a relationship for one person and know who the "other" person is.
            if the_person.identifier == self.person_a_identifier:
        ...
        def get_type(self, the_person = None):
            if the_person is None or the_person.identifier == self.person_a_identifier:
                return self.type_a
            elif the_person.identifier == self.person_b_identifier:
                return self.type_b
    ...
    class RelationshipArray(renpy.store.object):
        ...
        def get_relationship(self, person_a, person_b):
            for relationship in self.relationships:
                if (relationship.person_a_identifier == person_a.identifier and relationship.person_b_identifier == person_b.identifier) or (relationship.person_a_identifier == person_b.identifier and relationship.person_b_identifier == person_a.identifier):
        ...
        def get_relationship_list(self, the_person, types = None, visible = None):
            ...
            if isinstance(types, basestring):
                types = [types]
            for relationship in self.relationships:
                if (the_person.identifier == relationship.person_a_identifier and (types is None or relationship.type_a in types)) or (the_person.identifier == relationship.person_b_identifier and (types is None or relationship.type_b in types)): #What type we are looking at depends on if this is person A or B.
