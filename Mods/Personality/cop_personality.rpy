init 1400 python:
    ...
    def cop_possessive_titles(person):
        if person is police_chief:
            valid_possessive_titles = ["The police chief"]
            if person.love > 30:
                valid_possessive_titles.append("Your police chief")
        else:
            valid_possessive_titles = ["The police officer"]
            if person.love > 30:
                valid_possessive_titles.append("Your police officer")
        if person.sluttiness > 40:
    ...
    cop_personality = Personality("cop", default_prefix = reserved_personality.default_prefix,
        ...
        titles_function = cop_titles, possessive_titles_function = cop_possessive_titles, player_titles_function = reserved_player_titles)

label cop_introduction(the_person):
...
label cop_greetings(the_person):
...
label cop_grope_body_reject(the_person):
...
label cop_strip_obedience_accept(the_person, the_clothing, strip_type = "Full"):
...
label cop_flirt_response_low(the_person):
...
label cop_flirt_response_mid(the_person):
...
label cop_flirt_response_high(the_person):
    if mc.location.get_person_count() > 1 and the_person.effective_sluttiness() < (25 - (5*the_person.get_opinion_score("public_sex"))): # There are other people here, if she's not slutty she asks if you want to find somewhere quiet
        ...
        menu:
            "Find someplace quiet":
                ...
                call fuck_person(the_person, private = True, start_position = kissing, skip_intro = True) from _call_fuck_person_cop_response_high
        ...
        menu:
            "Kiss her":
                ...
                call fuck_person(the_person, start_position = kissing, private = mc.location.get_person_count() < 2, skip_intro = True) from _call_fuck_person_cop_response_high2
                $ the_person.call_dialogue("sex_review", the_report = _return)
                $ the_person.review_outfit()
